...
with mlflow.start_run(run_name="Viagens - Optuna"):
    run_data_analysis(df)
    x_train, x_val, x_test, y_train, y_val, y_test = run_data_preprocessing(df)
    run_model_tuning(x_train, y_train, x_val, y_val, x_test, y_test)
    mlflow.end_run()
